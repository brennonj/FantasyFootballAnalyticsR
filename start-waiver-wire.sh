#!/usr/bin/env bash
# Launches the waiver-wire add/drop tool at a fixed local URL.
#
#   ./start-waiver-wire.sh          this machine only
#   ./start-waiver-wire.sh --lan    also reachable from phones/tablets
#
# Stop it with Ctrl-C.
...
PORT=3840
APP="R Scripts/Waiver Tool/waiver_app.R"

HOST="${WAIVER_HOST:-127.0.0.1}"
for arg in "$@"; do
  case "$arg" in
...
if lsof -nP -iTCP:"$PORT" -sTCP:LISTEN >/dev/null 2>&1; then
  echo "Port $PORT is already in use - the waiver tool may already be running at $URL" >&2
  exit 1
fi

echo "Waiver wire  ->  $URL"
