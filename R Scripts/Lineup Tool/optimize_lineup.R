...
build_slot_pools <- function(slots) {
  ...
  if (flex_capacity > 0) {
    # Matched by base position (player_eligible_positions() in
    # espn_lineup_data.R reduces every player to real, atomic positions only -
    # e.g. a plain RB carries just "RB", never a compound flex code) - so a
    # real RB/WR/TE naturally matches this pool via the ordinary "%in%" below,
    # with no separate pseudo-position needed.
