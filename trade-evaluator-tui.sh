#!/usr/bin/env bash
# Launches the trade evaluator as a terminal UI - the same verdicts and
# recommendations as start-trade-evaluator.sh's web app, but with no HTTP
# server, so it works over a plain SSH session.
#
#   ./trade-evaluator-tui.sh
#
# Under the hood: an R process polls ESPN (pending trades every 5 min, full
# recommendation search every 30 min) and writes Data/trade_snapshot.json; a
# Python/Textual app renders that file and re-reads it every 2s. Stop with
# Ctrl-C or 'q' - this also stops the poller.
#
# To use this from another machine, SSH in and run this script normally:
#   ssh you@this-mac "cd ~/Developer/FantasyFootball && ./trade-evaluator-tui.sh"
...
VENV="Trade Evaluator TUI/.venv"
if [ ! -x "$VENV/bin/python" ]; then
  echo "Setting up the TUI's Python environment (first run only)..." >&2
  python3 -m venv "$VENV"
  "$VENV/bin/pip" install --quiet --upgrade pip
  "$VENV/bin/pip" install --quiet -r "Trade Evaluator TUI/requirements.txt"
fi

SNAPSHOT="Data/trade_snapshot.json"
rm -f "$SNAPSHOT"
...
exec "$VENV/bin/python" "Trade Evaluator TUI/trade_tui.py" "$SNAPSHOT"
